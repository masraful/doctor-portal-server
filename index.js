...
function verifyJWT(req, res, next) {
    ...
    const token = authHeader.split(' ')[1]; 2
...
async function run() {
    try {
        const appointmentOptionCollection = client.db('DrPortal').collection('Options');
        const bookingsCollection = client.db('DrPortal').collection('bookings');
        const usersCollection = client.db('DrPortal').collection('user')
        const doctorCollection = client.db('DrPortal').collection('doctors')
        const paymentsCollection = client.db('DrPortal').collection('payments')
        ...
        app.post('/bookings', async (req, res) => {
            ...
            if (alreadyBooked.length) {
                const massage = `You already have a booking on ${booking.appointmentDate}`;
